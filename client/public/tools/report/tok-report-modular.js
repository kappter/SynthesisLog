...
function redrawStoredCharts() {
  ...
  drawBarChart("termsChart", chartState.terms, { maxValue: 10, horizontal: true });
  drawBarChart("slotsChart", chartState.slots, { maxValue: 10, horizontal: false });
...
function drawLineChart(canvasId, series) {
  ...
  for (let g = 0; g <= 10; g += 2) {
    const y = top + innerH - (g / 10) * innerH;
    ctx.beginPath();
    ctx.moveTo(left, y);
    ctx.lineTo(left + innerW, y);
    ctx.stroke();
  ...
  const ys = series.points.map(p => top + innerH - ((p.value || 0) / 10) * innerH);
